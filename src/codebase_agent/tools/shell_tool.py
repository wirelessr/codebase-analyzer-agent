...
class ShellExecutionError(Exception):
    ...
    def __init__(self, command: str, exit_code: int, stderr: str, message: str = None):
        ...
        self.stderr = stderr
        super().__init__(message or f"Command '{command}' failed with exit code {exit_code}")
...
class ShellTool:
        ...
        if self.enable_logging:
            ...
            for pattern in complex_patterns:
                if pattern in command:
                    logger.info(f"Complex command pattern '{pattern}' detected: {command}")
                    break
    ...
    def validate_working_directory(self) -> List[str]:
        ...
        if not self.working_directory.exists():
            ...
            issues.append(f"Working directory is not a directory: {self.working_directory}")
        elif not self.is_working_directory_accessible:
            issues.append(f"Working directory is not accessible: {self.working_directory}")
