...
class TestAgentManager:
    ...
    def test_initialize_agents_success(self, mock_task_specialist_class, mock_code_analyzer_class, 
                                     mock_shell_tool_class, agent_manager):
        ...
        mock_code_analyzer_class.assert_called_once_with(expected_model_client, mock_shell_tool)
        mock_task_specialist_class.assert_called_once_with(expected_model_client)
    
    @patch('src.codebase_agent.agents.manager.ShellTool')
    @patch('src.codebase_agent.agents.manager.CodeAnalyzer')
    def test_initialize_agents_failure(self, mock_code_analyzer_class, mock_shell_tool_class, agent_manager):
    ...
    def test_process_query_accepted_first_review(self, agent_manager):
        ...
        review_result = (True, "Good analysis coverage", 0.85)  # (is_complete, feedback, confidence)
        ...
        # Execute
        result = agent_manager.process_query_with_review_cycle("test query", "/test/path")
        
        # Verify
        mock_code_analyzer.analyze_codebase.assert_called_once_with("test query", "/test/path", None)
        mock_task_specialist.review_analysis.assert_called_once_with(analysis_result, "test query", 1)
    ...
    def test_process_query_rejected_then_accepted(self, agent_manager):
        ...
        assert second_call[0] == ("test query", "/test/path", "Need more details on authentication")
    ...
    def test_synthesize_final_response_accepted(self, agent_manager):
        ...
        result = agent_manager._synthesize_final_response(analysis_result, True, feedback_message, original_query)
    ...
    def test_synthesize_final_response_forced(self, agent_manager):
        ...
        result = agent_manager._synthesize_final_response(analysis_result, False, feedback_message, original_query)
    ...
    @patch('src.codebase_agent.agents.manager.CodeAnalyzer')
    @patch('src.codebase_agent.agents.manager.TaskSpecialist')
    def test_full_review_cycle_mocked(self, mock_task_specialist_class, mock_code_analyzer_class, 
                                    mock_shell_tool_class, agent_manager):
