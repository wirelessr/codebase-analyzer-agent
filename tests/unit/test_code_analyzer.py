...
class TestCodeAnalyzer:
    ...
    def test_analyze_codebase_single_iteration_converges(self, mock_shell_exec, analyzer):
        ...
        async def mock_agent_run(task):
            mock_result = Mock()
            mock_result.messages = [Mock(content="""{
                "need_shell_execution": false,
                "shell_commands": [],
        ...
        with patch('asyncio.run') as mock_async_run:
            ...
            result = analyzer.analyze_codebase("What files are in this project?", "/test/path")
    ...
    def test_analyze_codebase_multiple_iterations(self, mock_shell_exec, analyzer):
        ...
        mock_shell_exec.return_value = [
            {"command": "ls -la", "success": True, "stdout": "file1.py\nfile2.py", "stderr": "", "error": None}
        ]
    ...
    def test_analyze_codebase_with_specialist_feedback(self, mock_shell_exec, analyzer):
        ...
        mock_shell_exec.return_value = [
            {"command": "grep -r 'class' .", "success": True, "stdout": "class TestClass:", "stderr": "", "error": None}
        ]
    ...
    @patch('codebase_agent.agents.code_analyzer.CodeAnalyzer._execute_shell_commands')
    def test_analyze_codebase_json_parsing_error_fallback(self, mock_shell_exec, analyzer):
        """Test analyze_codebase handles JSON parsing errors gracefully."""
        # Mock response with invalid JSON
        async def mock_agent_run(task):
            mock_result = Mock()
            mock_result.messages = [Mock(content="This is not valid JSON response from the LLM")]
            return mock_result
    ...
    def test_analyze_codebase_max_iterations_limit(self, mock_shell_exec, analyzer):
        ...
        mock_shell_exec.return_value = [
            {"command": "ls", "success": True, "stdout": "file.py", "stderr": "", "error": None}
        ]
    ...
    def test_build_iteration_prompt_includes_context(self, analyzer):
        ...
        shell_history = [{"iteration": 1, "results": [{"command": "ls", "success": True, "stdout": "file.py"}]}]
        shared_findings = ["Finding 1", "Finding 2"]
        convergence = {
            "sufficient_code_coverage": True,
        ...
        prompt = analyzer._build_iteration_prompt(
            query, codebase_path, iteration, context, shell_history, 
            shared_findings, convergence, specialist_feedback
        )
