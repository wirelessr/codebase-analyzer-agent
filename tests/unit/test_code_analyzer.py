...
class TestCodeAnalyzer:
    ...
    def test_execute_shell_commands_success(self, analyzer):
        ...
        commands = ["ls -la", "pwd"]
        analyzer.shell_tool.execute_command = Mock(side_effect=[
            (True, 'file1.py\nfile2.py', ''),  # Returns tuple (success, stdout, stderr)
            (True, '/test/project', '')
        ])
    ...
    def test_execute_shell_commands_failure(self, analyzer):
        ...
        analyzer.shell_tool.execute_command = Mock(side_effect=Exception("Test exception"))
    ...
    def test_assess_convergence_from_json_high_confidence(self, analyzer):
        ...
        context = [{'shell_results': [{'success': True}]}, {'shell_results': [{'success': True}]}]
    ...
    def test_assess_convergence_from_json_low_confidence(self, analyzer):
        ...
        assert convergence['sufficient_code_coverage'] == False  # len(context) < 2 and total_commands < 3
    ...
    def test_synthesize_final_response(self, analyzer):
        ...
        result = analyzer._synthesize_final_response(query, context, shared_findings, convergence)
