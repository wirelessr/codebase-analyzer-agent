...
"""
    ...
    def test_analyzer_dual_phase_execution(self, real_config, test_codebase):
        ...
        query = "Analyze the architecture of this Python project. I need to understand the system structure, component relationships, module organization, and overall design patterns used."
        ...
        assert (
            len(result) > 100
        ), f"Expected substantial architecture analysis result, got {len(result)} chars"
        ...
        if len(tracking_tool.calls) > 0:
            ...
            assert any(
                term in commands_str for term in exploration_terms
            ...
            assert any(
                term in result_lower for term in architecture_terms
            ), f"Expected architecture analysis to mention structural components, got: {result[:200]}..."

            print(
                "✅ Test passed: Dual-phase execution successfully analyzed project architecture!"
            )
        else:
            ...
            if len(result) > 50:
                ...
                print(
                    "⚠️  Test passed but no shell commands executed - LLM may have been overconfident about architecture"
                )
            else:
