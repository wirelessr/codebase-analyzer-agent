...
"""
            ...
            # Create test files
            with open(
                os.path.join(temp_dir, "tests/unit/test_user_model.py"), "w"
            ) as f:
                f.write(
                    """
import pytest
...
"""
    ...
    def test_knowledge_base_accumulation_across_iterations(
        ...
        assert (
            len(captured_iterations) >= 2
        ), f"Expected multiple iterations, got {len(captured_iterations)}"
        ...
        assert (
            len(knowledge_bases) >= 2
        ), "Should have captured knowledge bases from multiple iterations"
        ...
        assert len(last_kb) >= len(
            first_kb
        ), "Knowledge base should accumulate or refine findings"
        ...
        assert (
            len(found_terms) >= 3
        ), f"Expected data flow analysis terms, found: {found_terms}"

    def test_knowledge_base_refinement_and_updates(
        self, analyzer, complex_test_codebase
    ):
        ...
        if len(findings_evolution) >= 2:
            ...
            assert (
                database_related_count_later >= database_related_count_first
    ...
    def test_knowledge_base_persistence_and_continuity(
        self, analyzer, complex_test_codebase
    ):
        ...
        with patch.object(
            analyzer, "_build_iteration_prompt", side_effect=track_kb_usage
        ):
            result = analyzer.analyze_codebase(query, complex_test_codebase)
        ...
        if len(kb_timeline) >= 2:
            ...
            assert (
                len(non_empty_kbs) > 0
            ), "Should have at least some iterations with knowledge base content"
            ...
            if final_kb_size > 0:
                ...
                assert (
                    relevant_findings > 0 or final_kb_size == 0
    ...
    def test_knowledge_base_convergence_quality(self, analyzer, complex_test_codebase):
        ...
        found_keywords = [
            keyword for keyword in architecture_keywords if keyword in result.lower()
        ]

        # Should cover multiple architectural concepts
        assert (
            len(found_keywords) >= 3
        ), f"Analysis should cover architectural concepts, found: {found_keywords}"
    ...
    def test_knowledge_base_multilingual_handling(
        ...
        found_relevant = [
            keyword for keyword in relevant_keywords if keyword in result.lower()
        ]

        assert (
            len(found_relevant) >= 2
        ), f"Should contain relevant analysis, found: {found_relevant}"
